...
function absorbText(text) {
  ...
  text.split("\\n").forEach(function (line) {
    var heading = line.split("#")[0].match(/^\\s*\\[\\s*([^:\\]]+)/);
    if (heading) {
    ...
    if (entry) {
      ...
      (state.kept[port] = state.kept[port] || []).push({line: line, scene: scene});
    ...
    if (strip) {
      ...
      (state.stripKept[name] = state.stripKept[name] || []).push({line: line, scene: scene});
    }
  });
...
// The first word of an entry, which is the channel it drives. Two entries naming
// the same one in the same place is what the board rejects
function channelOf(line) {
  var word = line.split("#")[0].match(/^\\s*([^\\s:]+)/);
  return word ? word[1].toLowerCase() : "";
}

// Puts each kept line back where it came from: the always-on ones above the
// scenes, and a scene's ones under the heading of that name where the chosen look
// has one. A look whose scenes do not match has nowhere to put the rest, so the
// first stands for the channel and the others are dropped, which keeps the file
// valid where replaying them all as always-on would not be
function placeKept(entries, always, byScene, used) {
  (entries || []).forEach(function (kept) {
    var channel = channelOf(kept.line);
    if (kept.scene === null) {
      if (used[channel]) return;
      used[channel] = true;
      always.push(kept.line);
      return;
    }
    (byScene[kept.scene] = byScene[kept.scene] || []).push(kept);
  });
}

// Writes a scene's kept lines under its heading, and reports what found no home
function intoScenes(played, byScene) {
  var homeless = [];
  var lines = played.split("\\n");
  var out = [];
  var placed = {};
  lines.forEach(function (line) {
    ...
    var heading = line.split("#")[0].match(/^\\s*\\[\\s*([^:\\]]+)/);
    if (!heading) return;
    var name = heading[1].trim();
    (byScene[name] || []).forEach(function (kept) { out.push(kept.line); });
    placed[name] = true;
  });
  Object.keys(byScene).forEach(function (name) {
    if (!placed[name]) homeless = homeless.concat(byScene[name]);
  });
...
function currentText() {
  ...
  state.ports.forEach(function (port) {
    ...
    if (choice.mode === "keep") {
      placeKept(state.kept[port], always, byScene, used);
    } else {
      var line = screenLine(port, choice.media, pace);
      used[channelOf(line)] = true;
      always.push(line);
    }
  });
  state.strips.forEach(function (name) {
    ...
    if (!choice || choice.mode === "none") return;
    if (choice.mode === "keep") {
      placeKept(state.stripKept[name], always, byScene, used);
      return;
    }
  ...
  if (!state.outputs) played = withoutOutputs(played);
  var settled = intoScenes(played, byScene);
  // A scene the look does not have leaves its lines nowhere valid, so one stands
  // for the channel above the scenes and the rest go
  settled.homeless.forEach(function (kept) {
    var channel = channelOf(kept.line);
    if (used[channel]) return;
    used[channel] = true;
    always.push(kept.line);
  ...
  always.forEach(function (line) { parts.push(line + "\\n"); });
  var sound = soundLine();
