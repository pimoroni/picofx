# A spinny rainbow wheel, driven by the RGB565 reference driver.
#
# The RGB444 version of this example is the one to read first. This one exists to
# show the narrower driver working, and where it stops: the canvas has to be
# exactly the panel size, rotation is 0 or 180 only, and there is no pixel
# doubling or background fill. So it cycles four modes where the RGB444 example
# cycles sixteen.
#
# Each mode is held for MODE_FRAMES frames and printed, since a spinning wheel on
# its own does not tell you which one is active.
#
# Change the panel and PIN blocks to move this to another board.

from machine import SPI, Pin
from picovector import image, color, shape, mat3
from st7789_viper_rgb565 import ST7789_RGB565

# Panel and bus. 16-bit colour is a third more bytes per frame than 12-bit, so
# the same panel at the same baud rate takes a third longer to write.
WIDTH = 240
HEIGHT = 320
FRAME_RATE = 42
BAUDRATE = 24_000_000
...
# Every combination the driver handles. 180 degrees reverses both axes, so
# mirroring it leaves only the vertical flip.
MODES = [(rotation, mirror)
         for mirror in (False, True)
         for rotation in (0, 180)]

# What each mode does to the image, for the print below
EFFECT = {
    (0, False): "as drawn",
    (180, False): "both axes reversed",
    (0, True): "flipped horizontally",
    (180, True): "flipped vertically",
}

spi = SPI(id=SPI_ID, baudrate=BAUDRATE, sck=Pin(SCK_PIN), mosi=Pin(MOSI_PIN))
screen = ST7789_RGB565(spi, Pin(CS_PIN), Pin(DC_PIN), Pin(BL_PIN),
                      WIDTH, HEIGHT, FRAME_RATE)

# The canvas must match the panel exactly. These kernels have no placement
# arithmetic, so update() raises rather than guessing at a mismatch.
...
try:
    while True:
        ...
        rotation, mirror = MODES[index]
        if index != announced:
            announced = index
            print(f"mode {index + 1}/{len(MODES)}: rotation={rotation}, "
                  f"mirror={mirror} - {EFFECT[(rotation, mirror)]}")
        ...
        screen.update(canvas, rotation=rotation, mirror=mirror, v_sync=V_SYNC)
