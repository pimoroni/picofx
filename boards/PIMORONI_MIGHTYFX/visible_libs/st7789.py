...
#
# The ST7789 controller. Its register codes, the tables a panel is tuned from, and the
# bringup sequence. A second controller is a new module of the same shape.

import time
from collections import OrderedDict

# Every bit MADCTL carries. Only MADCTL_HORIZ_ORDER is written, the rest being the
# register's other abilities, defined for a future use rather than set here
...
# A frame's window, format and memory write
...
# Memory columns the controller holds, which with CONTROLLER_ROWS is the window
# covering any panel
...
def set_porch(screen, back, front):
    """Set a panel's blanking, back then front, in scan lines. One porch line is one
    line time, and the controller reads these as it enters blanking, so a write lands
    on the frame after it and cannot truncate the one in flight."""
    screen.command(REG_PORCTRL, bytes((back, front)) + PORCH_TAIL)

# Codes for the screen frame rate, the rate falling as the code rises
...
def reset(screen):
    """Reset a panel and wait out its settle. Apart from setup() so several panels can
    be reset through one broadcast, the settle paid once."""
...
def stop(screen):
    """Take a panel dark and put it to sleep. It keeps its memory, so left scanning the
    frame would come back the moment anything drove the backlight. Construction resets
    a panel, which wakes it."""
    screen.command(REG_DISPOFF)
    screen.command(REG_SLPIN)


def setup(screen, width, height, bitdepth_code, framerate_code, te=True):
    """Bring a panel up, which reset() must already have run on. te sends TEON so the
    panel drives its tearing-effect line, and panels sharing a DC line take te=False,
    since TEOFF still drives the line low and panels on one line would divide it."""
    screen.command(REG_TEON if te else REG_TEOFF)
    screen.command(REG_COLMOD, bitdepth_code)
    ...
    # One tuning for both panel types, measured better on the 1.54" than the 240x240
    # values its size would otherwise select, which clip to white near the top of the range
    ...
    # Always this value, since the scan direction does not follow MADCTL and a panel
    # flipped here tears. Rotation and mirroring are done in the frame path instead.
